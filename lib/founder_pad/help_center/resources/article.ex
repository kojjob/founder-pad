defmodule FounderPad.HelpCenter.Article do
  ...
  actions do
    ...
    create :create do
      accept [:title, :slug, :body, :excerpt, :help_context_key, :status, :position, :category_id]
    ...
    read :search do
      ...
      prepare fn query, _context ->
        ...
        query
        |> Ash.Query.filter(fragment("to_tsvector('english', coalesce(?, '') || ' ' || coalesce(?, '') || ' ' || coalesce(?, '')) @@ plainto_tsquery('english', ?)", title, excerpt, body, ^search_term))
