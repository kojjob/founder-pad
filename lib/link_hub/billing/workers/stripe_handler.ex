defmodule LinkHub.Billing.Workers.StripeHandler do
  ...
  defp notify_payment_failure(subscription) do
    case LinkHub.Accounts.Membership
         |> Ash.Query.filter(workspace_id == ^subscription.workspace_id)
         |> Ash.Query.filter(role == :owner)
         |> Ash.read() do
      {:ok, memberships} ->
        Enum.each(memberships, fn membership ->
          LinkHub.Notifications.Notification
          |> Ash.Changeset.for_create(:create, %{
