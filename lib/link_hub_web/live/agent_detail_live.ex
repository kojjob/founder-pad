defmodule LinkHubWeb.AgentDetailLive do
  @moduledoc "LiveView for viewing and interacting with an AI agent."
  ...
  def mount(%{"id" => agent_id}, _session, socket) do
    case Ash.get(LinkHub.AI.Agent, agent_id) do
      {:ok, agent} ->
        ...
        if connected?(socket) do
          Phoenix.PubSub.subscribe(LinkHub.PubSub, "conversation:#{conversation.id}")
        end
  ...
  defp compute_agent_stats(agent) do
    convos =
      case LinkHub.AI.Conversation |> Ash.Query.filter(agent_id: agent.id) |> Ash.count() do
        {:ok, n} -> n
        _ -> 0
      end
  ...
  defp load_activities(_agent_id) do
      ...
      {:ok, events} ->
        Enum.map(events, fn e ->
          %{
            ...
            action: e.event_name,
            entity: (e.workspace_id && String.slice(to_string(e.workspace_id), 0..7)) || "system",
            impact: "Recorded",
