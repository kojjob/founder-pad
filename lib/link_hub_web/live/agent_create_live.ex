defmodule LinkHubWeb.AgentCreateLive do
  @moduledoc "LiveView for creating new AI agents from templates or custom configuration."
  ...
  def handle_event("create_agent", %{"agent" => params}, socket) do
    ...
    if is_nil(org) do
      {:noreply, assign(socket, error: "No workspace found. Please complete onboarding first.")}
    else
      create_agent(socket, params, org)
    end
  end

  defp create_agent(socket, params, org) do
    ...
    case LinkHub.AI.Agent
      ...
      {:error, changeset} ->
        {:noreply, assign(socket, error: format_create_error(changeset))}
    end
  end
  ...
  defp format_create_error(_), do: "Failed to create agent. Please check all required fields."
  ...
  defp load_user_org(user) do
    case LinkHub.Accounts.Membership
         ...
         |> Ash.read() do
      {:ok, [membership | _]} -> membership.workspace
      _ -> nil
    end
  end
