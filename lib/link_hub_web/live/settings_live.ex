defmodule LinkHubWeb.SettingsLive do
  @moduledoc "LiveView for user and workspace settings management."
  ...
  def handle_event("save_avatar", _params, socket) do
    ...
    uploaded_files =
      consume_uploaded_entries(socket, :avatar, fn %{path: path}, entry ->
        ...
        dest = Path.join([:code.priv_dir(:link_hub), "static", "uploads", "avatars", filename])
        File.cp!(path, dest)
  ...
  defp parse_device(ua) when is_binary(ua) do
    Enum.find_value(@device_patterns, String.slice(ua, 0..30), fn {tokens, label} ->
      if Enum.all?(tokens, &String.contains?(ua, &1)), do: label
    end)
  end
  ...
  defp maybe_delete_old_avatar(url) when is_binary(url) do
    if String.starts_with?(url, "/uploads/avatars/") do
      path = Path.join([:code.priv_dir(:link_hub), "static", String.trim_leading(url, "/")])
