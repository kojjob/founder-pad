defmodule LinkHubWeb.TeamLive do
  @moduledoc "LiveView for team member management and invitations."
  ...
  def handle_event("delete_member", %{"id" => id}, socket) do
    case Ash.get(LinkHub.Accounts.Membership, id) do
      {:ok, membership} ->
        membership
        ...
        <div class="bg-surface-container rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
          ...
          <div class="px-8 pt-8 pb-4">
            ...
            <p class="text-xs text-on-surface-variant">
              Add up to 10 people at once. They must have a LinkHub account.
            </p>
          </div>
  ...
  defp process_invites(emails, org_id, role) do
    Enum.reduce(emails, {0, []}, fn email, {success, errors} ->
      case invite_user_by_email(email, org_id, role) do
        :ok -> {success + 1, errors}
        {:error, msg} -> {success, errors ++ [msg]}
      end
    end)
  end

  defp invite_user_by_email(email, org_id, role) do
    case LinkHub.Accounts.User |> Ash.Query.filter(email == ^email) |> Ash.read() do
      {:ok, [user | _]} ->
        case LinkHub.Accounts.Membership
             ...
             |> Ash.create() do
          {:ok, _} -> :ok
          {:error, _} -> {:error, "#{email}: already a member"}
        end
