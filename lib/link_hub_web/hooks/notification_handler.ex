defmodule LinkHubWeb.Hooks.NotificationHandler do
  @moduledoc """
  ...
  defp search_route(query) do
    Enum.find_value(@search_routes, "/activity", fn {keywords, path} ->
      if Enum.any?(keywords, &String.contains?(query, &1)), do: path
    end)
  end
  ...
  defp load_unread_notifications(user) do
    case LinkHub.Notifications.Notification
